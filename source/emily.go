...
func newAccount(host string, smtpPort uint64, imapPort uint64, uname string, password string, keyfile string) (*account, error) {
	...
	logDebug("newAccount created")

	go SlotGenerator(res.slot_chan)
	logDebug("SlotGenerator called")
	return res, nil
}
...
func (usr *account) rcv() ([][]byte, error) {
	...
	imapClient, err := client.DialTLS(usr.host+":"+strconv.FormatUint(usr.imapPort, 10), tlsconfig)
	if err != nil {
		logError("imap dial error:", err)
		return nil, err
	}
	defer imapClient.Logout()
	
	if err = imapClient.Login(usr.uname, usr.password); err != nil {
...
func (usr *account) sendMail(rcvrs []string, pld []byte) error {
	...
	smtpClient, err := smtp.Dial(servername)
	if err != nil {
		logError("cannot connect (smtp.Dial failed connecting to ", servername, "): ", err)
		return err
	}
	if err = smtpClient.Hello(usr.host); err != nil {
		logError(err)
		return err
	}
	...
	if err = smtpClient.Mail(usr.uname); err != nil {
		logError(err)
		return err
	}
